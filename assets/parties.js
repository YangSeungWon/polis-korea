// NEC sg_typecode → 직위명 단일 출처(전 페이지 공용). 폴 adapter·byelection·history·archive가 참조.
//   1=대통령 2=국회의원 3=광역단체장 4=기초단체장 5=광역의원 6=기초의원 7~9=비례 11=교육감.
//   주의: 무투표 API(fetch_uncontested 교육감=5)·축약 라벨(fetch_local '광역장')·'의원후보'(audit)는
//   목적이 달라 별개 — 이건 표준 sg_typecode↔표준 직위명 매핑만.
const TC_OFFICE = { '2': '국회의원', '3': '광역단체장', '4': '기초단체장', '5': '광역의원', '6': '기초의원', '11': '교육감' };
...
const PARTY_COLORS = {
  ...
  '민주통합당':       '#FFD400',  // 19대 총선 — 노랑 (namu.wiki)
  ...
  '민주자유당':       '#003990',  // 14대
  '신한국당':         '#003990',  // 15대 (민자당 후신·한나라당 전신) — 파랑
  ...
  '개혁신당':         '#FF7210',
  '진보당':           '#D6001C',
  '진보신당':         '#DC143C',  // 18-19대 — 빨강 (ko·namu 일치, 옛 #FF7A05 주황은 오류)
  ...
  '노동당':           '#FF0000',
  '새로운미래':       '#45BABD',
  '새미래민주당':     '#51BDC5',  // 새로운미래 개명(2024-09)
  '자유통일당':       '#1B5198',
  '공화당':           '#3F8FCB',
  '국민당':           '#FF6699',
  '국민의당':         '#006241',  // 안철수 — People Green
  '국민의당(2016)':   '#006241',  // 안철수 2016 — 그린
  '국민의당(2020)':   '#EA5504',  // 안철수 2020 — 주황
  '국민의당(1963)':   '#C2C247',  // 허정 1963 — 옛 군소(중립색)
  '바른미래당':       '#00B4B4',  // 21대 — 민트
  '국민통합21':       '#4F689A',  // 16대 (정몽준) — 네이비
  '민중당':           '#A5132F',
  '민중당(2017)':     '#F26522',  // 통진당 후신(민중연합당→민중당→진보당) — 오렌지
  '민주평화당':       '#43B02A',  // 2018 호남 평화당 — 생명·평화 녹색(창당 설문 결정)
  '새로운보수당':     '#5BC2E7',  // 2020 유승민 — 하늘색(2019-12 공개)
  '민생당':           '#0BA95F',  // 21대 비례 (구 평화당 등 통합) — 민생그린
  '기후민생당':       '#369173',  // 민생당 개명(2024-03)
  '민주평화당(2025)': '#00A85F',  // 기후민생당 개명(2025-04, 2018 평화당과 별개)
  '대한애국당':       '#3399FF',
  '녹색당':           '#7FBE26',
  '소나무당':         '#5DA532',
  '국가혁명당':       '#FFD700',
