...
(function () {
  ...
  async function main() {
    ...
    for (const [a, c] of edges) {
      ...
      // '기타' 계열은 컬럼 밖이라 x가 없다(아래 텍스트 목록으로 나간다). 그런 노드에
      // 닿는 선을 그리려 하면 d="MNaN,…"이 되어 SVG가 통째로 콘솔 에러를 낸다.
      // 실제로 자유당(2020)→자유민주당(2021) edge가 생기자 그렇게 됐다.
      // 그릴 수 없는 선은 그리지 않는다 — 두 끝이 다 컬럼에 있을 때만 잇는다.
      if (na.x == null || nc.x == null) continue;
