...
// 현재 활성 뷰의 뷰포트 핸들(포커스 capture/apply). 지도·시도 hex(#hex)·시군구 hex(#hex2).
function viewHandle(v) {
  if (v === 'map') return window.__mapViewport || null;
  if (v === 'hex') { const el = isSigunguMode() ? $('#hex2') : $('#hex'); return el && el.__svgViewport; }
  return null;
}
...
function setView(v) {
  ...
  else if (sigungu) Promise.resolve(renderSigunguHex()).then(applyFocus);
