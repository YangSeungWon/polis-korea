...
function propSystemInfo(n) {
  if (n === 9 || n === 10) return { label: '유정회', note: '유신정우회 — 선거 아님. 대통령이 추천하고 통일주체국민회의가 선출. 지역구 정수의 1/2.' };
  if (n <= 8) return { label: '전국구', note: '지역구의 약 1/3. 지역구 의석 비율로 배분하되 제1당이 과반 미달이면 1/2을 우선 배정. (정당 득표 아님)' };
...
function renderDetail() {
  ...
  if (state.selected) {
    ...
    const psido = periodSidoName(state.selected.sido, el && el.date);   // 옛 선거엔 강원도·전라북도·제주도
    const titleText = isSido ? psido : `${psido} · ${fmtUnitName(state.selected.name)}`;
...
async function init() {
  ...
  const [elections, hex, hexLegacy, hexPres, hexLocal, manifest] = await Promise.all([
    ...
    loadJson('data/geo/sigungu_hex_local.json').catch(() => null),
  ...
  state.hexPres = hexPres;   // 옛 대선 회차별 period 구 레이아웃 (회차→셀). 2~18대.
  state.hexLocal = hexLocal; // 지선 기초장 회차별 period 시군구 레이아웃 (회차→셀). 1~9회.
