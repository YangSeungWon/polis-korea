...
//   6~9회(2014~2026)→2025(SGIS 2025-2Q 동→시군구, 상세). 이전엔 매핑 없어 sigungu_simple
//   (30정점/폴리곤 과단순화)로 폴백해 해안선이 엉기설기했음. 시군구 세트는 2018~2025 안정 →
//   2025 한 파일로. 부천분구·청주통합·군위 시도이동은 reverse-merge/sido-history로 흡수.
const LOCAL_SGG_GEO_YEAR = { 1: 1995, 2: 2000, 3: 2002, 4: 2006, 5: 2010, 6: 2025, 7: 2025, 8: 2025, 9: 2025 };

// 대선 시군구 geo — 15대(1997)부터. (13·14대는 1995 도농통합 전 시군구라 맞는 경계 파일 없음.)
//   15대1997→2000(울산광역시 O·도농통합 후라 ≈1997; 김포·안성 시승격·여천→여수는 name-history로 흡수)
...
// 19~21은 이전 sigungu_simple(과단순화 30정점/폴리곤)이 엉기설기해 2025로 교체.
// 시-단위 유닛은 2018~2025 안정 → 한 파일로 19~21 공용. 일반구는 reverse-merge로 집계.
//   2~7대(1952~71)→1975(가장 오래된 SGIS 경계; 선거구 갑/을은 자치구 폴리곤에 self-merge)
//   13대1987→1985  14대1992→1990 (SGIS 전체 버전 시군구). 8~12대는 체육관 간선이라 geo 불가.
const PRES_SGG_GEO_YEAR = {
