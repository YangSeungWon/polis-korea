...
const SIGUNGU_SIDO_HISTORY = {
  ...
  '울진군': ['경상북도', '강원특별자치도'],  // 1963 강원 → 경북 (2~4대 대선 강원)
  '금산군': ['충청남도', '전북특별자치도'],   // 1963 전북 → 충남 (2~4대 대선 전북; canonSido 비교라 전북특별자치도)
};
...
const SIGUNGU_NAME_HISTORY = {
  ...
  // 옛 대선 geo 경계명(1975~1990) → 그 회차 데이터명. 시·군 분리·승격·개명·신설로 어긋난 회색 해소.
  '명주군': ['강릉군'], '원성군': ['원주군'], '춘성군': ['춘천군'],  // 강원 시·군 미분리/개명(춘성→춘천군 1992)
  '안양시': ['시흥군'], '의정부시': ['양주군'],                      // 경기 시 승격 전(안양1973·의정부1963)
  '거제군': ['통영군'], '삼천포시': ['사천군'], '충무시': ['통영군'],  // 경남 — 1952 거제=통영, 승격 1955
  '진해시': ['창원군'], '의창군': ['창원군'], '울산시': ['울주군', '울산군'],
  '신안군': ['무안군'],                                             // 전남 — 1969 무안서 분리 전
  '금성시': ['나주시', '나주군'],                                    // 전남 — 금성시(1981~86)=나주
  '중원군': ['충주군'], '천원군': ['천안군'],                        // 충북 충주·충남 천안 군부 개명 전
  '월성군': ['경주군'],                                             // 경북 — 월성군(1955~88)=경주 군부
};
...
const SIGUNGU_HEX_LIFECYCLE = {
  ...
  // 인천 자치구 개편 — 중구(육지)+동구→제물포구, 중구 영종도→영종구, 서구 북부→검단구.
  // 9회 지선(2026-06-03)은 신설 자치구 단위로 실시 — 결과 데이터가 영종구·제물포구·검단구로
  // 들어옴. 그래서 since=선거일(2026-06-03): 9회는 새 이름 그대로(데이터 매칭됨), 8회 이전
  // 회차만 옛 행정구역으로 거슬러 표시. 영종도=옛 중구, 제물포=옛 동구, 검단=옛 서구.
  '인천광역시|검단구':   { since: '2026-06-03', beforeAs: { sido: '인천광역시', name: '서구' } },
  '인천광역시|영종구':   { since: '2026-06-03', beforeAs: { sido: '인천광역시', name: '중구' } },
  '인천광역시|제물포구': { since: '2026-06-03', beforeAs: { sido: '인천광역시', name: '동구' } },
