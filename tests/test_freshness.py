...
def main() -> int:
    ...
    print("\n[예정일] 휴간을 건너뛴다")
    # 휴간 뒤로 밀 때 **이튿날이 아니라 한 주기 뒤**다. 이튿날로 밀면 휴간 종료
    # 다음 날을 예정일로 잡아, 실제 첫 발표가 오기도 전에 overdue가 뜬다.
    # 갤럽은 화~목 조사하고 금요일에 내므로 휴간 종료(8/7) 직후 발표는 없다.
    nxt = next_expected(D("2026-07-25"), 10, "gallup")
    ck(f"7/25 + 10일 → 8/17 (휴간 종료 + 한 주기, 실제 {nxt})",
       nxt == D("2026-08-17"), str(nxt))
