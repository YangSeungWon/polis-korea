...
2. **동 경계** — 통계청 SGIS 읍면동 경계(연도별 1975~2025, UTM-K 5179→WGS84). 선거연도 최근접 센서스.
   최신 경계(2020·2022·2025)는 data.go.kr 공공데이터포털 15129688에서 쿠키 없이 직접 다운(시도·시군구·행정동 일괄).
...
### 5. 읍을 리(里) 단위로 쪼개는 선거구 — 동-union 한계 (미처리)
우리 파이프라인은 **행정동/읍/면 단위**라 한 읍을 리 단위로 둘로 가르는 선거구는 못 그림.
전국에 극소수: **21대 화성시 봉담읍**(헌정 사상 최초 읍 분할 — 분천·왕림·세곡·당하·마하·유·덕·
덕우·하가등·상기리 → 갑, 상·내·수영·동화·와우·수기리 → 병. 21대 한정 예외) + 부산 중구 중앙동.
- 영향: **생산 회차(18·19대 등) 분할 동 0~1개**(봉담은 21대뿐). 21대 생산 지도는 OhmyNews 권위본
  사용(이미 리 분할 처리). OhmyNews 대조 시 화성병 IoU 0.58로 남으나 검증 한정·생산 무영향.
- NEC VCCP08 투표구도 '봉담읍제N투'로 번호만 있어 리 배정 불가. 처리하려면 별표의 리 목록 +
  리(법정동)/집계구 경계 필요(SGIS bnd_jojung·vworld) → 일회성 예외 대비 과투자라 미처리로 둠.
- **부천과 근본 차이 — 최신 경계로도 해결 불가**: 부천은 필요한 단위(일반구·작은동)가 SGIS 2025에
  실재해 경계 교체로 풀렸지만(빈티지 문제), 봉담읍은 SGIS **2022·2025 모두 단일 폴리곤**이라 어느 연도
  경계로도 리-분할 표현 불가(해상도 문제, 리<읍이 SGIS 체계에 없음). OhmyNews 21대는 별표 리 목록으로
  봉담읍을 화성갑 55%/병 44%로 직접 쪼갬 — SGIS-union 방식으론 재현 못 함. 생산 21대=OhmyNews라 실제
  지도엔 정상, 내 복원본 검증에서만 화성병 0.58.
- 그 외 IoU 낮은 도심 자치구(마포·강서·영등포 등)는 **동 배정은 권위본과 정확히 일치**, 기하 차이는
  동 경계 소스 차이(통계청 SGIS ↔ VW-Lab)일 뿐 — 오류 아님(작은 자치구라 IoU로 증폭).
...
- **평균 IoU 0.917 · 중앙 0.929**, IoU>0.9 = 173/253, >0.8 = 242/253. (~92% 일치)
- 서로 다른 동 소스(통계청 SGIS ↔ VW-Lab)인데 92% 일치 → 동-union 방법 타당, 9~20대 복원 신뢰.
- ※ 이 대조 과정에서 **파서 버그 발견·수정**: SKIP_ROW의 `투표`가 면 투표구 'X면투표소'를 특수행으로
  오인해 통째 스킵 → 18·19·21대(2008+ 'X투표소' naming) 면 누락. 진주 IoU 0.56→0.97 등으로 회복,
  전체 평균 0.889→0.917. 18·19 생산 지도 재빌드(갑/을 경계 정확화).
- 남은 IoU<0.8(11개)은 대부분 **도심 자치구 갑/을/병**(화성병·서울 마포/강서/영등포·부산 사하·
  안산단원·인천 연수) — 동 단위로 쪼개는 경계라 두 동 소스 미세 차이가 가장 크게 드러남.
- **22대도 대조(SGIS 2022 근사)**: 평균 IoU **0.963**·중앙 0.970, >0.9 248/254. 이 과정에서 2건 보강:
  ① 전북·강원 **특별자치도 승격**으로 NEC cityCode 변경(4200/4500→5200/5300) → fetch SIDO_OVERRIDE[22].
  ② **군위군 2023 경북→대구 이동** — SGIS 2022엔 경북이라 안 잡힘 → 빌드에 **dong_global**(전국 유일
  동명이면 시도 무관 매칭) 추가 → 군위 IoU 0.21→0.944/0.988. 일반적 '이동 시군구' 처리(9~20엔 ~1동
  영향으로 무시할 수준).
- **부천 일반구 부활 — SGIS 2025로 해결**: 부천은 2019 광역동(대동)→2024-01 일반구·37동 복귀. SGIS 2022는
  아직 광역동이라 NEC 2024 작은동(상1동 등)이 안 잡혀 부천 IoU 0.55였음. **SGIS 2025-2Q 경계**(2024-01 부활
  반영, data.go.kr 15129688 직접 다운)로 CFG[22]를 교체 → 동 매칭실패 0, 부천 **갑0.969·을0.957·병0.945**,
  전체 최저 0.904로 회복. 복원 로직은 본디 정확했고 빠진 건 경계 빈티지뿐이었음. 생산 22대는 OhmyNews 유지.
