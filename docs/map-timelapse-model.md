...
`REGIONS` 상수를 지웠다. **events.json이 아는 변화 지점 전부**가 대상이고, 연결 성분을
kind별로 묶어 지역을 만든다. 손으로 관리하면 새 사건을 넣고 목록에 적는 걸 잊는 순간
조용히 빠진다.

지금 5곳: 포항(통합) · 하남(분구) · 부천(부분 통합) · 이천(승격) · 군위(시도 이관).

### 전국으로 가서야 드러난 것

**① 개명은 합산이 아니다.** 이천군→이천시를 `aggregated`로 냈었다. 더한 게 없는데
합산이라고 부르면 안 된다 — 사건이 이미 `comparison_capability: direct`라고 말했다.

**② 영역이 그대로인 사건도 사건이다.** 승격·이관은 폴리곤이 **안 바뀌는 게 맞다**.
"사건 전후로 경계가 바뀐다"고 뭉뚱그려 검사하면 이천·군위가 틀린 것으로 잡힌다.
`pivot_event.expect_topology_change`(= `territorial_continuity != same`)가 어느 쪽인지
말해 주고, 화면도 "영역은 그대로이고 이름·소속만 바뀌었습니다"라고 말한다.

**③ 경계 그림이 없는 시점이 있다.** 모델은 21대 부천시갑을 아는데 20대 선거구 그림에는
대응이 없다. 빌드를 세우는 대신 `boundary_resolution`을 셋으로 나눴다:
`exact` / `predecessor_same_territory`(영역이 그대로일 때만 옛 이름으로 이어 붙임) /
`unavailable`(폴리곤도 색도 내지 않고 그 사실을 말함). 빌드는 **모델에 없는 폴리곤을
그렸을 때만** 세운다.

**④ 진단이 자료보다 먼저 나오면 안 된다.** 22대 군위군은 선거구가 '동구군위군을'이라
밖으로 삐져나오지만, 같은 선거의 **시군구 단위 실측**(`scope: district_sigungu`)이
따로 있어 그대로 칠할 수 있다. 깨끗한 덮개를 먼저 찾지 않고 실패를 먼저 보고해서
'표시 불가'가 나왔다. 순서를 뒤집었다.

**⑤ 이름으로 series를 가르면 이름 없는 행을 놓친다.** 시군구별 집계 파일의 비례 행에는
`name`이 없어 지역구로 분류됐고, 군위군 비례 38개 정당이 `general:district`에 섞였다.
`sg_typecode`가 정본이다(7 = 비례). `region_timeline.series_id`도 같이 고쳤다.

**⑥ `.sigungu.json`을 빼면 안 된다.** 16대 대선부터 시군구 집계는 거기에만 있다 —
본 파일은 전국·시도 18줄뿐이다. 빼놓았더니 군위 대선 series가 통째로 사라졌다.
대신 (선거·series·단위)를 **먼저 준 파일만** 쓴다.

**⑦ 갑·을은 그 자체가 정보다.** '동구군위군갑'을 군위군 폴리곤에 칠하면 군위군이 통째로
갑에 들어간다고 주장하는 셈이다. 끝의 갑·을·병·정을 `partial` 신호로 읽는다.

**⑧ 다른 판인 series는 뺀다.** '군위군선거구'(광역의원)는 쪼개도 행정구역이 안 나오고,
'하남시제1선거구'는 국회의원 선거구가 아니다. 맞물리는데 못 칠하는 것(포항 총선)과
아예 다른 판인 것은 다르다 — 앞은 남겨서 배우고, 뒤는 `excluded_series`에 이유를 적고
뺀다. **조용히 사라지면 자료가 없는 것처럼 보인다.**

## 다음

`reaggregated`가 실제로 성립하는 사례(level 허용)로 확장.
지금은 그 분기를 함수에 직접 물려서 동작만 확인한다 — **있는 것처럼 데이터를 만들지
않는다**. 그리고 events.json에 사건을 더 넣는 만큼 대상 지역이 자동으로 늘어난다.
