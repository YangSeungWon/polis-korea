...
- viz `tracker.js` 3개 섹션: ②-b 정당지지 성·연령별(renderPartyDemo, renderPartySupport 재사용),
  ②-c 무당층(renderUndecided, 없음·모름 합·집단별 한 선), ②-d 정당별 성별 격차(renderGenderGap,
  남·여 짝지어 남−여 0기준). archive에도 출구조사 기반 '성별 격차'(덤벨)·'직전 swing' 뷰.
...
일요일 KST 06:47 (또는 workflow_dispatch 수동) →
  scrape_nesdc --gubun VT012           # 커밋된 CSV 기준 증분 — 신규 폴만
  ...
  → nesdc_etc_polls.csv + approval_*.json·aggregated_etc.json 변경 시 commit·push
```

- `concurrency: data-pipeline` — daily-refresh와 같은 그룹(raw 동시접근 방지·큐잉).
- `timeout-minutes: 60` — 증분이라 수 분이면 끝남(아래 cold-start 교훈 참고).
- 캐시 `data/raw/pdf`, key `tracker-pdf-<run_id>`, restore-keys `tracker-pdf-`(daily와
  분리 — 거대한 daily 캐시를 안 끌어옴). 신규 PDF 재다운로드만 회피하는 보조 역할.

### cold-start 교훈 — 왜 CSV를 커밋하는가

첫 설계는 cache miss 시 Release(`raw-bundle-v1`)에서 PDF를 복원했는데, Release엔
**9회 PDF만 있고 VT012가 없어** cold run이 VT012 2,600여 건을 0부터 재다운로드하다
**5h 타임아웃 초과로 실패**했다(2026-06 첫 시험). 근본 해법:

- **`data/raw/nesdc_etc_polls.csv`를 git에 커밋**(`.gitignore` 예외) → `scrape_nesdc`의
  `load_existing_ids`가 항상 동작 → cache miss여도 **신규 폴만** 스크랩.
- 추출기는 `--incremental` + 커밋 JSON의 `done` → **신규 PDF만** 파싱.
- 게다가 CI엔 etc CSV만 존재 → `load_meta()`가 etc ntts만 반환 → 추출 대상이 VT012로
  자연 한정(9회 base PDF 무시).
...
| **커밋된 CSV** (`data/raw/nesdc_etc_polls.csv`) | ✓ git | 없음 | VT012 목록 — scrape 증분의 기준 |
| **Actions 캐시** (`tracker-pdf-`) | △ 임시 | evict돼도 신규만 재다운로드 | 신규 PDF 재다운 회피(보조) |
| **로컬 raw** (`data/raw/*`) | ✓ 전체 ~6.8GB | 없음(gitignore) | 과거 코퍼스·full 재추출용 |

로컬 ↔ CI 동기화는 **커밋된 CSV + JSON**. (Release `raw-bundle-v1`은 daily 전용 —
tracker는 안 씀.)
...
- 커밋 CSV 덕에 cache miss여도 scrape는 항상 증분 → **cold run도 수 분**(매주 안정).
...
> 한 줄: **데이터 양·신선도는 자동으로 잘 자라고 잃지 않는다(cold run도 증분이라 가벼움).
> 다만 품질 개선의 소급 적용은 로컬 없이는 불가능.**
