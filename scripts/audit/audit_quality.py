...
사용:
  python3 scripts/audit/audit_quality.py             # 전체 검사 + 저장
  python3 scripts/audit/audit_quality.py --no-save   # 화면만 (CI 모드)
  python3 scripts/audit/audit_quality.py --strict    # warn도 exit 1
...
def check_outliers(polls: list, roster: dict, window_days: int = 21) -> dict:
    """1. 시간 windowed 시계열 outlier.

    같은 (시도·시군구·office·후보) 시리즈에서 자신의 시점 ±window_days 윈도우 안
    다른 조사들과만 median 비교. 그 윈도우에 3+ 샘플 있어야 검사 (단발·격동 직후
    분리된 시점은 skip). Δ > max(15, 4*MAD)이면 outlier.

    이전에 5개월 차이 16878 담양군 12월 조사가 5월 데이터와 묶여 outlier로 잡히던
    것 해결. 시점 격동(1-2주 ±20pp 변동) 정상 variation은 자기 윈도우 안에서만
    비교되어 검출 안 됨.
    """
    ...
    for key, samples in series.items():
        if len(samples) < 3:
            continue
        # 각 sample의 ±window_days 윈도우 내 비교
        for self_ed, self_ntt, self_pct in samples:
            window = [pct for ed, _, pct in samples
                      if abs((ed - self_ed).days) <= window_days]
            if len(window) < 3:
                continue
            med = median(window)
            ...
            if mad < 2.0:
                continue
            dev = abs(self_pct - med)
            if dev > max(15, 4 * mad):
                hits.append({
                    "ntt": self_ntt, "key": list(key), "pct": self_pct,
                    "window_median": round(med, 1), "window_mad": round(mad, 1),
                    "window_n": len(window), "dev": round(dev, 1),
                    "period_end": str(self_ed),
                })
    return {"name": "시계열 outlier (시간 windowed)", "severity": "warn" if hits else "info",
