...
각 시도 = 직사각형 (또는 거의). 만 없음, cluster 연결성 보장.

⚠️ **내부 빈칸 0은 보장하지 않는다.** 오래 그렇게 적혀 있었지만 사실이 아니다 — 영남
70셀을 이 구조(대구 3×3 · 울산 3×2 L · 경북 wrap · 경남/부산 bot)로 놓으면 빈칸 0인
후보가 탐색 공간에 아예 없고 최소가 2다. 그 하나가 바깥으로 못 나가고 갇힌 게
sigungu_hex의 (12,10)이었다(이웃: 대구 동구·울산 북구·경북 영천시·청송군).
경북이 공간을 더 받은 게 아니다 — 22칸으로 실제 시군구 수와 같다. 모양의 문제다.

그래서 **이 스크립트를 돌린 뒤 반드시** 구멍을 메운다:

    python scripts/build/fill_district_hex_holes.py --apply --path data/geo/sigungu_hex.json

시도 연결성을 지키는 slide-fill이라 셀 수·시도 소속은 그대로고 좌표만 1칸씩 움직인다.
