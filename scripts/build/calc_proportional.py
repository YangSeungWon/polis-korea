"""비례 (tc=8 광역의원 · tc=9 기초의원) 의석 배분 — 헤어식 (largest remainder).

⚠ tc=9(기초의원 비례)는 FALLBACK 전용 — 확정 명부 우선.
  이 추정은 시군구별 비례정수(8회 baseline 룩업)·5% 봉쇄·2/3 상한을 직접 계산하는데,
  baseline 정수가 실제와 달라 9회 기준 298석으로 과소집계됨(실제 384). 또 1석 시군구는
  2/3 상한 cap=int(1×2/3)=0 버그로 1위 정당 의석을 2위에 넘겼었음(현재 수정).
  **확정 당선인 명부로 교체**: 5~8회 fetch_council_prop.py(OpenAPI),
  9회 fetch_council_winners_live.py(EPEI01). tc=8 광역비례는 명부 미연동이라 이 계산 유지.
